...
const FormSocmed: React.FC = () => {
  const cookies = new Cookies()
  let storage: any = {
    socmeds: cookies.get('user_socmed') ? JSON.parse(decodeURI(cookies.get('user_socmed'))) : user.socmed,
  }

  const [state, setState] = useState({
    socmeds: storage.socmeds.map((item: any) => ({...item, show: false})),
    showAdd: false
  })

  const onSubmit = (data: any) => {
    data.preventDefault();
    const setter = state.socmeds.map((item: any) => ({
      name: item.name,
      title: item.title,
      target: '',
      link: item.link
    }))
    cookies.set(`user_socmed`, encodeURI(JSON.stringify(setter)), { maxAge: 864000, path: '/' })
    storage = {...state}
  }

  const onChange = (item: any, e: any) => {
    const select = [...state.socmeds].findIndex((data: any) => data === item)
    const key = e.target.id.replace(`__${select}_`, '')
    setState((prev: any) => {
      prev.socmeds[select][key] = e.target.value
      return {...prev}
    })
  }

  const showing = (item: any) => {
    const key = [...state.socmeds].findIndex((data: any) => data === item)
    setState((prev: any) => {
      prev.socmeds[key].show = !item.show
      return {...prev}
    })
  }

  const deleting = (key: number) => {
    const socmeds = [...state.socmeds].filter((item: any, index: number) => index !== key)
    setState((prev: any) => {
      prev.socmeds = socmeds
      return {...prev}
    })
    storage = {...state}
  }
  ...
  return (
    <>
      ...
      <form
        ...
        {
          state.socmeds.map((item: any, key: number) => (
            <div key={key} className={key === state.socmeds.length - 1 && item.show ? '' : 'border-b border-black border-opacity-30 pb-8'}>
              ...
              <div className={`${item.show ? 'h-auto ' : 'h-0 overflow-hidden '}transition-all ease-in-out duration-200 space-y-6 md:w-2/3 mx-auto`}>
                ...
                <div>
                  <div className="text-sm font-poppins mb-1">
                    <label htmlFor={`__${key}_title`} className="capitalize">
                      Title
                    </label>
                  </div>
                  <div className="relative bg-gray-800 bg-opacity-30">
